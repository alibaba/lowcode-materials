      ...
      onNodeAdd: (dragment, currentNode) => {
        ...
        const wrapWithBlock = (dragment, node, dropTargetName, blockLen) => {
          setTimeout(() => {
            ...
            const newNode = node.document.createNode(layoutBlockNode(blockLen).schema);
            node.insertBefore(newNode, dragment, false);
            dragment.remove(false);
            newNode.getChildren().get(0).getChildren().get(0).getChildren().get(0).select();
          }, 1);
        };
        ...
        const wrapWithP = (dragment, node, dropTargetName) => {
          setTimeout(() => {
            ...
            const newNode = node.document.createNode(Object.assign(layoutPNode.schema));
            node.insertBefore(newNode, dragment, false);
            dragment.remove(false);
