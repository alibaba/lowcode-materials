...
import setter_cn from './setter'
import './components/index'

GlobalRegistry.registerDesignerLocales(setter_cn)
...
const mapEnum = (dataSource: any[]) => (item: any, index: number) => {
  ...
  return {
    ...
    label: label?.label ?? label ?? 'Unknow'
    // label: icon ? (
...
export const useLocales = () => {
  onFieldReact('*', (field) => {
    ...
    const takeMessage = (prop?: string) => {
      ...
      return GlobalRegistry.getDesignerMessage(token)
    }
