...
```js
{
  "plugins": [
    [
      'import',
      {
        libraryName: 'disscode-lib',
        customName: name => `disscode-lib/lib/components/${name.slice(3)}`,
        customStyleName: name => `disscode-lib/dist/style/components/${name.slice(3)}.scss`
      },
      'disscode-lib'
    ]
  ]
...
// 编译后代码
import AtButton from 'disscode-lib/lib/components/button'
...
// app.js
import 'disscode-lib/dist/style/index.scss' // 全局引入一次即可
```
:::

**引入组件样式的三种方式**

- **全局引入（JS中）：** 在入口文件中引入 `disscode-lib` 所有的样式
:::demo
```js
import 'disscode-lib/dist/style/index.scss' // 引入组件样式 - 方式一
```
:::
...
:::demo
```scss
@import "~disscode-lib/dist/style/index.scss"; // 引入组件样式 - 方式二
```
:::
...
:::demo
```scss
@import "~disscode-lib/dist/style/components/button.scss"; // 引入所需的组件样式 - 方式三
```
:::

> 具体的组件样式文件请查看 [组件样式列表](https://github.com/NervJS/disscode-lib/tree/dev/src/style/components)
...
:::demo
```scss
@import "~disscode-lib/dist/style/index.scss"; // 引入组件样式，仅需引入一次即可
```
:::
