...
class SwiperItem extends React.Component {
  render() {
    ...
    return (
      <div className={cls} style={style} item-id={itemId} {...restProps}>
        {children}
      </div>
    );
  }
...
class Swiper extends React.Component {
  ...
  componentDidMount() {
    ...
    const that = this;
    console.log('[preventClicksPropagation] ');
    const opt = {
      // 指示器
  ...
  componentWillReceiveProps(nextProps) {
    if (this.mySwiper) {
      const nextCurrent =
        typeof nextProps.current === 'number' ? nextProps.current : this._$current || 0;
