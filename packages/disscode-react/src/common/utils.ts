...
const createSelectorQuery = Taro.createSelectorQuery || require('@tarojs/taro').createSelectorQuery;
...
function delayQuerySelector(selectorStr: string, delayTime = 500): Promise<any[]> {
  return new Promise((resolve) => {
    let selector: SelectorQuery = createSelectorQuery?.();
    delay(delayTime).then(() => {
      if (!selector) selector = createSelectorQuery?.();
      selector
        .select(selectorStr)
        .boundingClientRect()
        .exec((res: any[]) => {
